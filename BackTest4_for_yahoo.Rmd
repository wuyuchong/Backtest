                    ...
                    proportion_profit = 0, fix_profit = 0)
{
  ...
  for(i in 3:nrow(dat))
  {
    pre_high = c(pre_high, dat$high[i-1] + (dat$high[i-1] - dat$high[i-2] + dat$low[i-1] - dat$low[i-2]) / 2)
    pre_low = c(pre_low, dat$low[i-1] + (dat$high[i-1] - dat$high[i-2] + dat$low[i-1] - dat$low[i-2]) / 2)
  }
  ...
  # Plot: forcast
  p = ggplot(dat, aes(x = trade_date, y = close)) +
    geom_candlestick(mapping = aes(open = open, high = high, low = low, close = close),
                     colour_down = "darkgreen", colour_up = "red", fill_up = "red", fill_down = "darkgreen") +
    geom_point(mapping = aes(x = trade_date, y = pre_high), color = "red") +
    geom_point(mapping = aes(x = trade_date, y = pre_low), color = "blue")
  ...
  if(method == "beginning")
  {
    for(i in 2:nrow(dat))
    {
      if(dat$open[i] < dat$threshold[i] & dat$open[i] > dat$pre_low[i])
      {
  ...
  # Return
  outcome = list("data" = dat, "plot" = p, "sum_accuracy" = sum_accuracy, "sum_trigger" = sum_trigger, "sum_margin" = sum_margin, "p_margin" = p_margin, "p_points" = p_points)
  return(outcome)
}
```
...
```{r}
stock_code = "AAPL"
stock_start = "2010-02-01"
stock_end = "2010-04-01"
...
```{r, include=FALSE}
knitr::opts_chunk$set(fig.pos = 'H', echo = FALSE, message = FALSE, warning = FALSE, eval = FALSE, include = FALSE)
```
...
```{r}
stock_code = "AAPL"
stock_start = "2016-05-01"
stock_end = "2016-11-01"
