...
output:
  rticles::ctex:
    ...
    toc: yes
date: 2020-02-12
categories: ["投资", "国际市场", "回测", "理论研究"]
tags: ["投资", "国际市场", "回测", "理论研究"]
...
1. 该策略为纯技术分析，已包括了手续费、印花税和滑点。

2. 使用了从 2010-01-01 到 2019-12-31 的十年市场的历史数据进行策略回测。

3. 使用了中国、香港、美国的股票和 ETF 进行回测。

4. 该策略仅为理论情形，不代表实盘情况，只具备研究意义。

```{r}
BackTest = function(stock_code, stock_start, stock_end, fee = 0.006,
                    stock_interval = 'weekly', adj = TRUE, permit = FALSE)
{
  ...
  sum_margin = data.frame(margin_compound = round(compound[length(compound)], 2),
...
```{r}
stock_code = c("QQQ", "TLt", "GDX", "^IXIC", "^GSPC", "^DJI", "^RUT", "AAPL", "MMM", "ABT", "ABBV", "ABMD", "ACN", "600519.SS", "000002.SZ", "159901.SZ", "600019.SS", "601857.SS", "0941.HK", "2328.HK", "0241.HK", "0215.HK")
```
...
```{r}
kable(list$sum_margin, digits = 2)
```
