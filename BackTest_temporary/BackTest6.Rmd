...
```{r}
BackTest = function(stock_code, stock_start, stock_end, fee = 0.0075, method = "fluct",
                    stock_interval = 'weekly', adj = TRUE, permit = FALSE)
{
  ...
  for(i in 2:nrow(dat))
  {
    ...
    if(dire == "flat")
    {
      if(dat$open[i] < dat$low[i-1])
      {
        ...
        trade_cost = c(trade_cost, dat$open[i])
      }
  ...
  trade = data.frame(trade_date, trade_dire, trade_kind, trade_cost, trade_sell, trade_sell_date)
  ...
  colnames(trade) = c("date", "direction", "kind", "cost", "sell", "sell_date", "margin", "margin_rate")
...
stock_code = c("QQQ", "SPY", "TLt", "GDX", "^IXIC", "^GSPC", "^DJI", "^RUT", "AAPL", "MMM", "ABT", "ABBV", "ABMD", "ACN", "600519.SS", "000002.SZ", "159901.SZ", "000001.SZ", "600019.SS", "601857.SS", "0941.HK", "2328.HK", "0241.HK", "045340.KQ", "0215.HK")
frame = c()
for(i in stock_code)
{
